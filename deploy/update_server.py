...
MODULES_DIR = Path(os.environ.get("GAMECENTER_UPDATE_MODULES_DIR", str(BASE_DIR / "modules")))
APP_DIR = Path(os.environ.get("GAMECENTER_UPDATE_APP_DIR", str(BASE_DIR / "app")))
LEGACY_DIR = BASE_DIR / "downloads"
PUBLIC_BASE_URL = os.environ.get("GAMECENTER_UPDATE_PUBLIC_BASE_URL", "https://hk-update.tcp0053.shop").rstrip("/")
...
class UpdateHandler(BaseHTTPRequestHandler):
    ...
    def handle_request(self, head_only=False):
        ...
        if path == "/version-beta.json":
            self.send_bytes(200, json_bytes(build_version_payload("beta")), "application/json; charset=utf-8", head_only=head_only)
            return

        if path == "/version-release.json":
            self.send_bytes(200, json_bytes(build_version_payload("release")), "application/json; charset=utf-8", head_only=head_only)
            return
        ...
        if path in ("/version.json", "/downloads/version.json", "/app/version.json"):
            ...
            self.send_bytes(200, json_bytes(payload), "application/json; charset=utf-8", head_only=head_only)
            return

        # ===== /modules/* 必须在 .apk 检查之前，否则 /modules/xxx.apk 会被误路由 =====
        if path == "/modules.json":
            modules_file = BASE_DIR / "modules.json"
            if not modules_file.exists() or not modules_file.is_file():
                self.send_json(404, {"ok": False, "error": "modules.json not found"}, head_only=head_only)
                return
            self.send_file(modules_file, "application/json; charset=utf-8", head_only=head_only)
            return

        if path.startswith("/modules/"):
            rel = Path(unquote(path.lstrip("/")))
            if ".." in rel.parts:
                self.send_json(400, {"ok": False, "error": "bad path"}, head_only=head_only)
                return
            module_file = MODULES_DIR / rel.name
            if not module_file.exists() or not module_file.is_file():
                self.send_json(404, {"ok": False, "error": "module not found"}, head_only=head_only)
                return
            content_type = "application/json; charset=utf-8" if module_file.suffix == ".json" else "application/octet-stream"
            self.send_file(module_file, content_type, head_only=head_only)
            return
        ...
        if path.startswith("/ai-models/"):
            ...
            if ".." in rel.parts:
                self.send_json(400, {"ok": False, "error": "bad path"}, head_only=head_only)
                return
            model_file = APP_DIR / rel
            if not model_file.exists() or not model_file.is_file():
                self.send_json(404, {"ok": False, "error": "not found"}, head_only=head_only)
                return
            content_type = "application/json; charset=utf-8" if model_file.suffix == ".json" else "application/octet-stream"
            self.send_file(model_file, content_type, head_only=head_only)
            return

        if path in ("", "/"):
            self.send_bytes(200, b"GameCenter HK update service OK\n", "text/plain; charset=utf-8", head_only=head_only)
            return
