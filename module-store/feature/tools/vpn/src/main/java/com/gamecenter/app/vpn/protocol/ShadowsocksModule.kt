...
class ShadowsocksModule : ProtocolModule {

    private val TAG = "ShadowsocksModule"
    ...
    override fun prepare(node: Node): Boolean {
        if (node.type != ProtocolType.Shadowsocks) return false
        if (node.address.isEmpty() || node.port <= 0 || node.password.isNullOrEmpty()) return false
        currentNode = node
        return true
    }
    ...
    override fun connect(): Pair<InputStream, OutputStream> {
        ...
        socket = Socket(node.address, node.port)
        connected = true
    ...
    override fun disconnect() {
        ...
        socket = null; connected = false; currentNode = null
    }
