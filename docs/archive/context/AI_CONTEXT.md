...
- 公共网络模块（`com.GameMatrix.app.network`）：所有联机游戏共享
- 其余 20+ 款游戏为单机模式
...
**重点模块**：
1. **公共联机网络模块** — `com.GameMatrix.app.network` 包，包含 GameSocketClient/Server/LANManager/RelayHttpClient/RemoteP2PUtil/OnlineChatHelper/OnlineRoomManager
...
5. **APK 签名模块** — keystore.properties + GameMatrix.keystore + signingConfigs（已修复）
...
### 2.4 斗地主 / com.GameMatrix.app.games.doudizhu
...
以下 4 个游戏支持云联机，使用公共网络模块 `com.GameMatrix.app.network`：
...
### 2.8 工具箱 / com.GameMatrix.app.tools
...
### 2.9 自定义 View / com.GameMatrix.app.views
...
- **Keep 规则**：`com.GameMatrix.app.**` 包下的所有类已配置 keep 规则，防止被混淆
- **原因**：游戏注册、反射调用、JNI 交互等场景需要保持类名/方法名不变

### 4.3 模块调用约定

- **GameRegistry 双轨注册**：游戏 Activity 类可通过 @GameEntry 注解自声明元数据，也可在 GameRegistry 中静态硬编码或动态注册
- **联机游戏共享网络模块**：所有联机游戏使用 `com.GameMatrix.app.network` 包中的公共网络组件；推荐使用 `OnlineRoomManager` 组合式复用，而非继承 `BaseOnlineActivity`
...
- **不要提交签名文件到 Git** — `GameMatrix.keystore` 和 `keystore.properties` 包含敏感信息，已添加到 `.gitignore`
