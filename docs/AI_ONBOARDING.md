...
| VPS | 地址 | 用�?| 公网域名 |
|-----|------|------|---------|
| 香港 VPS | `149.104.29.181` | 主更新源 | `https://your-server.example.com` |
| ~~美国 VPS~~ | ~~备用~~ | ~~备用更新源~~（2026-06-19 已下线） | ~~`https://your-server.example.com:1443`~~ |
...
~~local_private/服务器部�?upload_config_us.json~~（2026-06-19 已废弃）
```
...
### 3.6 上传后验�?
确认以下 URL 可访问：
- `https://your-server.example.com/version-beta.json`
- `https://your-server.example.com/version-release.json`
...
# 2. 确认 local_private/服务器部�?upload_config_hk.json 存在且正�?# 3. 确认 paramiko 已安�?python -m pip install paramiko
```
...
├── local_private/                     # 本地私有配置（不�?Git�?�?  ├── 服务器部�?
�?  �?  ├── upload_config_hk.json
    └── upload_config_us.json          # 已废弃（2026-06-19 US VPS 下线）
�?  └── github/
�?      └── token.txt
...
1. **先看本文�?*，了�?服务器部�?GitHub/构建/模块市场全貌
2. **复用现有脚本**（`工具/upload_to_vps.py`、`工具/upload_to_github_release.py`），不要重新发明流程
3. **遵守模块市场约束**（第 7 节），特别是 builtIn 规则�?downloadUrl 验证
