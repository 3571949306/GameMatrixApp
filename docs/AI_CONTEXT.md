...
## 2026-05-27 v1.3 性能基准测试

### 性能监控模块 (PerfMonitor)

**文件位置**：`app/src/main/java/com/gamecenter/app/monitor/PerfMonitor.java`

**功能**：
1. **冷启动时间测量**
   - 记录启动开始时间（System.nanoTime()）
   - 提供 `recordFirstFrame(View)` 方法记录首帧渲染完成时间
   - 计算冷启动时间 = 首帧时间 - 启动时间（毫秒）
   - 目标：< 2000ms

2. **内存占用监控**
   - 使用 HandlerThread 后台线程，每 5 秒采样一次
...
3. **帧率监控**
   - 使用 Choreographer 的 FrameCallback 监听每一帧
   - 通过帧时间间隔计算 FPS：`FPS = 1_000_000_000 / 平均帧间隔`
   - 保存最近 300 帧的时间戳用于计算
   - 目标：> 55 FPS

4. **性能报告生成**
   - 在 `stopMonitoring()` 中自动生成 JSON 格式报告
   - 报告包含：冷启动时间、内存峰值/平均值、FPS 平均值/最小值、总体评估
...
// 在首帧渲染完成后（如 Activity 的 onWindowFocusChanged）
PerfMonitor.getInstance().recordFirstFrame(getWindow().getDecorView());
```

### 性能基准测试 (PerfMonitorTest)

**文件位置**：`app/src/test/java/com/gamecenter/app/monitor/PerfMonitorTest.java`

**测试覆盖**（13 个测试用例 + 3 个基准测试）：
1. **功能测试**：单例模式、启动/停止监控、冷启动测量、内存采样、FPS 计算、JSON 报告格式、性能目标判断、边界情况处理、弱引用防泄漏
...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| 冷启动时间 | < 2000ms | Application.attachBaseContext → 首帧渲染完成 |
| 内存占用 | < 200MB | Java Heap + Native Heap 峰值 |
| 帧率 (FPS) | > 55 | 使用 Choreographer 计算平均帧率 |

### 已知问题

- **TD-03 未处理**：RePlugin 桥接层 Android 15+ 兼容性未验证
- **测试覆盖率未达标**：当前 29.3%，目标 > 40%
- **connectedAndroidTest 无法执行**：需要配置 Gradle 离线模式或内部 Maven 仓库

### 下一步计划 (v1.4)

1. **TD-03**：RePlugin 桥接层 Android 15+ 兼容性测试
2. **测试覆盖率提升**：配置 Gradle 离线模式，使 connectedAndroidTest 可以执行
3. **模块依赖自动下载功能**：实现 ModuleManifest.dependencies 解析（TD-05）
