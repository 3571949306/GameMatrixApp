# :core:moduleloader 模块

动态模块加载引擎（`DexClassLoader` + 校验 + 热更新）。
...
modules.json（VPS）
  ↓
ModuleDownloadManager（下载 APK 到缓存）
  ↓
...
- ⚠️ 证书固定（`SecureOkHttpFactory`）只在网络层使用；模块本身走的是 APK 签名验证。

## 不要做

- ❌ 不要跳过 `ModuleVerifier` 直接 `load`（生产环境会出安全事故）。
- ❌ 不要在主线程跑 `loadModule`（`DexClassLoader` 较慢，会卡 UI）。
- ❌ 不要假设模块有 `Manifest` 中声明的 Activity（动态加载的 Activity 走 `Fragment` 承载）。

## 参考

- Phase 1 设计笔记见 [`docs/项目改进建议书.md`](../../docs/项目改进建议书.md)。
- 模块商店元数据格式：`modules.json` schema（VPS 同步）。
