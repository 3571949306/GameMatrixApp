...
4. [核心模块详解](#4-核心模块详解)
   ...
   - 4.8 [数据库(database / di)](#48-数据库)
   - 4.9 [Fragment 层](#49-fragment-层)
   - 4.10 [工具层(utils)](#410-工具层)
5. [服务端组件](#5-服务端组件)
6. [依赖关系图](#6-依赖关系图)
7. [构建与发布](#7-构建与发布)
8. [测试体系](#8-测试体系)
...
GameMatrixApp 是一款 Android 综合应用，内置 **2 款经典游戏**，支持通过**模块市场**按需下载扩展游戏、**网络工具箱**、**浏览器**、**AI 助手**、**科学上网（VPN）**。应用采用单 Activity + 多 Fragment + Navigation Component 架构，底部导航栏根据已安装模块动态显示。
### 核心特性
| 功能 | 说明 |
|--------|------|
...
```
┌─  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──┐
│                       Android App                           │  │                                                             │  │  ┌─  ──  ──  ──  ──  ─┐ ┌─  ──  ──  ──  ──  ─┐ ┌─  ──  ──  ──  ──  ─┐ ┌─  ──  ──  ──  ──  ──  ─┐     │  │  │ Games    │  │Browser*  │  │ Tools*   │  │   AI*      │  │
...
│  └─  ──  ──  ──  ──  ─┘└  ──  ──  ──  ──  ──┘└  ──  ──  ──  ──  ──┘└  ──  ──  ──  ──  ──  ──┘        │  └─  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──  ──┘
          │  │
┌─  ──  ──  ──  ──  ──  ──  ──  ──┐ ┌─  ──  ──  ──  ──  ──  ──  ──  ──┐
│ VPS 更新服务端  │  │ WebSocket 中转  │  │ (Python)        │  │ (Node.js)       │  └─  ──  ──  ──  ──  ──  ──  ──  ──┘ └  ──  ──  ──  ──  ──  ──  ──  ──  ─┘
```

> *标记的 Fragment 仅在对应模块安装后显示。*
### 架构模式

- **UI 层**：单Activity + Navigation Component + Fragment（KeepStateNavigator add/show/hide 策略）
- **业务层**：ViewModel + LiveData（更新模块），Manager 模式（其他模块），逐步迁移。
- **数据层**：Room 数据库 + SharedPreferences + 加密存储 (EncryptedSharedPreferences)
- **依赖注入**: Hilt (Dagger) 统一管理单例，`@Inject` 构造函数优先，`getInstance()` 已标注 `@Deprecated`
...
│  │  │  │  │  └─  ─ {各游戏子包}/           # 7 款内置 + 19 款市场下载游戏实现│  │  │  │  ├  ── initializers/              # App Startup 初始化器│  │  │  │  │  └─  ─ NetworkInitializer.java
│  │  │  │  ├  ── network/                   # 网络与联机模块  │  │  │  │  ├  ── BaseOnlineActivity.java # 联机基类（模板方法）
...
│  │  │  │  │  ├─  ─ RelayClientHelper.java # 中转客户端 │  │  │  │  │  ├─  ─ RelayHostHelper.java   # 中转主机
...
│  │  │  │  ├  ── settings/                  # 设置对话框│  │  │  │  │  └─  ─ AppSettingsDialog.java
│  │  │  │  ├  ── 工具/                     # 网络工具层│  │  │  │  │  ├─  ─ ToolBinder.java        # 工具绑定接口
│  │  │  │  │  ├─  ─ ToolHelper.java        # 工具辅助类│  │  │  │  │  ├─  ─ ToolSection.java       # 工具分区
│  │  │  │  │  ├─  ─ ToolSectionStore.java  # 分区存储
│  │  │  │  │  ├─  ─ AdvancedToolBinders.java
│  │  │  │  │  └─  ─ {各工具Binder}/        # 20+ 工具实现
│  │  │  │  ├  ── update/                    # 更新模块
│  │  │  │  │  ├─  ─ UpdateManager.java     # 更新管理器（门面模式）│  │  │  │  │  ├─  ─ UpdateChecker.java     # 更新检查│  │  │  │  │  ├─  ─ UpdateDownloader.java  # 下载管理
│  │  │  │  │  ├─  ─ UpdateInstaller.java   # 安装管理
│  │  │  │  │  ├─  ─ UpdateInfo.java        # 更新信息模型
│  │  │  │  │  ├─  ─ UpdatePresenter.java   # 更新展示器│  │  │  │  │  ├─  ─ UpdateNotificationHelper.java
│  │  │  │  │  └─  ─ SSLHelper.java         # SSL 信任
│  │  │  │  ├  ── utils/                     # 通用工具
│  │  │  │  │  ├─  ─ ErrorReporter.java     # 错误上报
│  │  │  │  │  ├─  ─ I18nHelper.java        # 国际化│  │  │  │  │  ├─  ─ NetworkErrorHandler.java
...
│  │  └  ── androidTest/                       # 集成测试
│  └─  ─ build.gradle                           # 应用构建配置
├  ── 服务器部署/                               # 服务端组件│  ├─  ─ ddz_ws_relay/                  # WebSocket 中转服务│  ├─  ─ server.js
│  │  └  ── package.json
│  └─  ─ var_www_update/                # 更新服务│  ├─  ─ update_server.py
│  ├─  ─ ddz_relay/
│  └─  ─ feedback/
├  ── 工具/                             # 构建与发布工具 │  ├─  ─ upload_to_vps.py
...
├  ── version.properties                 # 版本号配置└  ── .github/workflows/ci.yml          # CI/CD 配置
```
...
- `scanAnnotatedGames(Context)` �?扫描 APK 中标注 `@GameEntry` �?Activity
...
| 工具层| ToolsModuleEntryPoint.kt | tools |
...
| 模式 | 协议 | 适用场景 |
|------|------|---------|
| NSD (优先) | mDNS/DNS-SD (`_doudizhu._tcp.`) | Android 设备份|
| UDP 广播 (备份 | JSON 报文 + 255.255.255.255 | NSD 不可用时 |
...
**内部类** `RetryInterceptor` �?线性退避策略（delay × attempt+1），最多重3 次
---
...
| 文本编解密| TextCodecToolBinder | Base64/URL 编解密|
| 设备信息 | DeviceToolBinder / SystemInfoToolBinder | 设备/系统信息 |
| 传感�?| SensorToolBinder | 传感器数据读取|
...
| `DownloadState.Verifying` | 校验证|
| `DownloadState.Completed(apkFile)` | 下载完成 |
| `DownloadState.Error(message)` | 下载出错 |
| `DownloadState.Cancelled` | 下载取消 |
| `CheckResult`（密封的| 检查结果：Success/NoUpdate/BetaOnly/BetaBlocked/Error |
| `DownloadResult`（密封的| 下载结果：Success/Verifying/Error/Cancelled |
...
- `enableBetaAndRecheck(context)` �?开Beta 并重新检查- `installApk(context)` �?安装 APK
...
```
UpdateManager (门面)
    ├  ── UpdateChecker      �?检查更新（网络请求 + 版本比对战    ├  ── UpdateDownloader   �?下载 APK（断点续�?+ MD5 校验证    ├  ── UpdateInstaller    �?安装 APK（权限检查+ FileProvider�?    └  ── UpdateNotificationHelper �?通知管理
```

**DI 变更**: 构造函数已标注 `@Inject`，Hilt 可直接创建实例。`getInstance()` 已标注 `@Deprecated`�?
### 安全加固
...
每套方案包含完整�?Material Design 3 色彩体系（浅�?+ 深色双模式），通过 `applyScheme()` 应用Activity 级别，或通过 `applySchemeToView()` 应用到单�?View�?
---
...
| `OkHttpClientProvider` | ~~`getInstance()`~~ �?`@Inject` 构造函数 | 已移至`@Provides` |
| `OkHttpClient` | �?Provider 获取 | 无变化 |
| `SettingsManager` | ~~`getInstance()`~~ �?`@Inject` 构造函数 | 已移至`@Provides` |
| `UpdateManager` | ~~`getInstance()`~~ �?`@Inject` 构造函数 | 已移至`@Provides` |
| `AiPreferences` | 直接构�?| 无变化 |
| `AppDatabase` | `getDatabase(context)` | 无变化 |
| `AiMessageDao` | �?Database 获取 | 无变化 |
| `GameStatsDao` | �?Database 获取 | 无变化 |
| `SaveManager` | ~~`getInstance()`~~ �?`@Inject` 构造函数 | 已迁移到 Kotlin，移至`@Provides` |
| `ErrorReporter` | `getInstance(context)` | 待迁�?|

---

### 4.9 Fragment 层
应用使用 Navigation Component 管理四个 的
| Fragment | 导航 ID | 功能 |
|----------|---------|------|
| `GamesFragment` | navigation_games | 游戏大厅（TabLayout + RecyclerView 卡片网格�?|
| `BrowserFragment` | navigation_browser | 内置浏览�?|
| `ToolsFragment` | navigation_tools | 网络工具层|
...
游戏大厅，核心功能：
- **TabLayout**: 全部 / 最�?/ 收藏 / 各分类标注- **搜索**: 实时过滤游戏名称、描述和分类
...
- `AppError.fromHttpCode(Int)` �?�?HTTP 状态码映射为对应错误的- `isNetworkError()` / `isServerError()` / `isRecoverable()` �?错误分类判断
...
- �?10 分钟清理空房间（超过 1 小时
**HTTP 端点**:
- `GET /health` �?健康检查- `GET /stats` �?统计信息（房间数、运行时间、内存）
...
App (Application)
    ├  ── SettingsManager
    ├  ── ColorSchemeManager
    └  ── OkHttpClientProvider (App Startup 延迟初始化器
...
5. APK 大小报告（计�?APK 大小并写入`GITHUB_STEP_SUMMARY`。6. 上传 Debug APK (保留 30 �?
7. 运行单元测试
8. 测试结果报告（解密XML 测试报告，输出通过/失败/跳过统计�?`GITHUB_STEP_SUMMARY`。9. 上传测试报告 (保留 7 �?
10. Android Lint 执行 (`lintDebug`)
11. Lint 问题报告（解密Lint 输出并上传结�?artifact�?
---
...
| **双重检查锁* | SettingsManager, OkHttpClientProvider, AppDatabase 单例初始化器|
...
- **getter 方法**：Java �?`getXxx()` 方法Kotlin 中可用属性语�?`xxx` 访问，但 `hasXxx()` / `isXxx()` 除外——仅�?Java 方法名符�?JavaBeans 规范时才能用属性语法。例�?`hasUpdate()` 不符合规范（应为 `isHasUpdate()`），必须�?`info.hasUpdate()` 显式调用�?- **可空�?*：Java 类型�?Kotlin 中为平台类型（`Type!`），需显式处理可空性。推荐在 Java 代码中添�?`@Nullable` / `@NonNull` 注解密- **受检异常**：Kotlin 不强制处理Java 受检异常，但建议在调用Java 方法时仍�?try-catch 包裹可能抛出异常的代码- **数组**：Java 数组�?Kotlin 中映射为 `Array<T>`，基本类型数组映射为 `IntArray`、`ByteArray` 等
### 10.4 迁移优先�?
新增代码优先使用 Kotlin 的场景：
...
- 修复 CloudFlare CDN 缓存APK 文件响应（改文件名绕过缓存）
- 彻底分离 VPS 更新服务9000）与模块商店服务9001），nginx 路由分离
- ModuleManager 重构：loadModuleList 实现本地优先 + 后台版本对比 + 无条件本地兜�?
## 2026-05-23 文档同步：科学上网VPN 模块上线

- 新增科学上网 VPN 模块（vpn_basic），首个非内置可下载模块（builtIn=false）�?- VPN 代码全在 feature/vpn 独立模块中，编译�?APK 上传�?VPS modules/ 目录- �?APK 仅保留VpnServiceProxy（~70�?TUN 隧道�? ModuleShellFragment（~70�?动态宿主）�?- core:common 新增三个接口：ModuleInterface、FeatureModule、VpnDelegate�?- ModuleShellFragment 复用为通用动态模块宿主，未来新增动态模块无需重复开发宿主逻辑�?- 修复 VPS update_server.py 路由顺序：`/modules/` 检测移至`.apk` 之前，避免模�?APK 被误路由�?- modules.json �?vpn_basic 条目已更新：entryClass、downloadUrl、sha256、fileSize 全部填写真实值- VPN 模块 dex 文件已上传至 `/var/www/update/modules/feature_vpn_v100.apk`�?62KB）�?
## 2026-05-23 文档同步：模块市场架构
- 新增模块市场（ModuleStoreActivity），入口位于游戏大厅左上角版本号下方�?- 初始安装包仅内置7款经典游戏，其余19款游戏改为市场下载模块- 浏览器、工具箱、AI助手改为独立市场模块，初始安装包不再自带�?- 底部导航栏动态化：仅显示"游戏"Tab，安装浏览器/工具层AI模块后自动出现对应Tab�?- 游戏分类精简�?个：经典（内置）、益智（市场下载）、休闲（市场下载）�?- ModuleManifest扩展5个字段（type/activityClass/gameId/gameCategory/gameDesc）�?- ModuleManager新增registerInstalledGameModules()和registerGameFromManifest()。- VPS更新服务器新增modules.json�?modules/路由�?- 新增3个模块入口类：BrowserModuleEntryPoint、ToolsModuleEntryPoint、AiModuleEntryPoint�?- ModuleManifest新增builtIn字段，内置模块显示启用"按钮而非"下载"�?- 修复模块市场下载失败：所有模块都指向不存在的dex文件，改为builtIn=true无需下载�?- 修复模块市场界面被状态栏遮挡：添加fitsSystemWindows=true�?## 2026-05-23 文档同步：模块市场分类与已安装列�?
- 模块市场新增分类Tab和已下载模块列表
