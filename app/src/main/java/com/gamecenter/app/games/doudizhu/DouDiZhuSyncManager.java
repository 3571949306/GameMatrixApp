 ...
 *
 * <p>你可以把这个类想象成"广播站"——房主每次做出游戏变更后，
 * 广播站就把最新状态发出去，还要确认每个听众都收到了。
 * 如果有人没收到，就再播一遍。</p>
 *
 * <p>关键设计决策：
 * <ul>
 *   <li>采用递增版本号（stateVersion）机制，客户端通过版本号判断状态是否过期
 *       （就像报纸的"期号"，看期号就知道是不是最新的）</li>
 *   <li>SYNC_STATE 广播采用"立即发送+两次延迟重发"策略（0ms/180ms/600ms），
 *       提高弱网可靠性（重要消息说三遍，确保对方听到）</li>
 *   <li>客户端动作通过 actionId 去重，防止网络重传导致重复处理
 *       （就像"已读"标记，同一条消息不处理两次）</li>
 *   <li>每个远程座位收到的 SYNC_STATE 包含该座位专属的手牌和记牌器数据，
 *       避免信息泄露（你只能看到自己的牌，不能看到别人的）</li>
 * </ul>
 */
