# GameMatrixApp VPS Update And Feedback
...
# 指定 APK 和版本文件路径
python 工具/upload_to_vps.py --apk app/build/outputs/apk/debug/app-debug.apk --version app/build/outputs/apk/debug/version.json --channel beta
```

配置文件在本机私有目录 `local_private/服务器部署/upload_config.json`，当前使用 SSH 密码登录。不要把该目录提交到 GitHub。
